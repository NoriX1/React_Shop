...
import { avaliableSizes, availableTypes } from '../constants';
...
const Cart = () => {
  ...
  const renderProducts = () => {
    return Object.keys(itemsByProps).map(key => {
      const [id, type, size] = key.split('_');
      return (
        <CartItem
          key={key}
          name={items[id].name}
          size={avaliableSizes[size].size}
          type={availableTypes[type].type}
          count={itemsByProps[key].count}
      ...
      <div className="cart">
        ...
        <div className="cart__bottom">
          ...
          <div className="cart__bottom-buttons">
            <Link to="/" className="button button_outline button-add go-back-btn">
